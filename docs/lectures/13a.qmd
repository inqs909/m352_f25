...
How do you stop the algorithm?

## Multivariate Newton-Raphson

$$
\boldsymbol \beta^{(it+1)} = \boldsymbol \beta^{(it)} - \frac{G_{\boldsymbol \beta}^{(it)}}{H_{\boldsymbol \beta}^{(it)}}
$$

-   $\boldsymbol \beta^{(it)}$: current estimate of $\boldsymbol \beta$

-   $G_{\beta_j}^{(it)}$: Gradient

-   $H_{\beta_j}^{(it)}$: Hessian Matrix

-   $\boldsymbol \beta^{(it+1)}$: Updated estimate of $\boldsymbol \beta$
